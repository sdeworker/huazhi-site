...
function djEmergenceOk(t){
  if(!t) return false;
  // 先剥掉否定式（"不做裁决""绝不互补"是守禁令，不是组装）
  const clean = t.replace(/(不|非|别|莫|绝不|不是|不做|不许|不搞|禁止|严禁)[^，。；\n]{0,10}?(裁决|互补|组装|叠加|拼接|各有道理|补盲|补上|谁赢|谁更)/g,'');
  // 升温/相变的证据（放宽：只要有一处真张力或涌现宣告即可）
  const heat = /互相攻击|互相不服|反驳|撕|顶回|顶死|似乱|相变|涌现|不属于(其中)?任何|单臂?(都)?(没有|到不了|不可达)|烧出|现造|三股|逼出/.test(t);
  // 组装的证据（收紧：只认"明确的组装宣言"，不被正常比喻误伤）
  const assembly = /(哪条|谁|C\d)\s*(最)?(更接近真相|更对|更准|胜出|赢了|说得对)|三者都对(所以)?(一起|都要)|各有道理.{0,6}(结合|综合|一起)|A\s*\+\s*B|把三(条|份|者)(合|拼|凑)(起来|一起)/.test(clean);
